...
!!! Add the starting value for the 1st DAX time series!!!




Let's have a look, how we can recover a single DAX time series from only a limited number of eigenvectors. To obtain the original data we need to multiply the transpose eigenvector matrix with PC scores from the left-hand side - as proofed earlier. But we can also restrict ourselves to the first n eigenvectors and the first n PC scores to produce an approximation. 

- We try to reproduce the first DAX intraday chart that we have already seen above.
- Using all 165 eigenvectors gives us the exact original data.
- Taking only the first 75 eigenvectors into account, we can roughly recognize the original series.
- But using only the first 10 eigenvectors does not fit the original series at all.

```{r warning=FALSE}
# recover 1st DAX series from all 165 eigenvectors
recov_165 <- t(eigenvectors)[,1:165] %*% pc_scores[1:165,]
...
p_recov_165 <- ggplot(vec, mapping = aes(x=minute, y=vec_165$recov_165_s1_diff_demean_acc)) +
  geom_line() + 
  xlab("minute") + 
  ylab("1st DAX series recovered from 165 evecs")

p_recov_165


# recover 1st DAX series from 75 eigenvectors
recov_75 <- t(eigenvectors)[,1:75] %*% pc_scores[1:75,]
...
p_recov_75 <- ggplot(vec, mapping = aes(x=minute, y=vec_75$recov_75_s1_diff_demean_acc)) +
  geom_line() + 
  xlab("minute") + 
  ylab("1st DAX series recovered from 75 evecs")


# recover 1st DAX series from only 10 eigenvectors
recov_10 <- t(eigenvectors)[,1:10] %*% pc_scores[1:10,]
...
p_recov_10 <- ggplot(vec, mapping = aes(x=minute, y=vec_10$recov_10_s1_diff_demean_acc)) +
  geom_line() + 
  xlab("minute") + 
  ylab("1st DAX series recovered from 10 evecs")
...
# plot first 4 Eigencharts on a grid
p1 <- ggplot(vec, mapping = aes(x=minute, y=vec1$eigenchart_1)) +
  geom_line() + 
  xlab("minute") + 
  ylab("1st DAX Eigenchart")

p2 <- ggplot(vec, mapping = aes(x=minute, y=vec2$eigenchart_2)) +
  geom_line() + 
  xlab("minute") + 
  ylab("2nd DAX Eigenchart")

p3 <- ggplot(vec, mapping = aes(x=minute, y=vec3$eigenchart_3)) +
  geom_line() + 
  xlab("minute") + 
  ylab("3rd DAX Eigenchart")

p4 <- ggplot(vec, mapping = aes(x=minute, y=vec4$eigenchart_4)) +
  geom_line() + 
  xlab("minute") + 
...
# second part: start over with all 165 charts
# can they be clustered? are there similarities? But: In terms of what? Idea:
# one dimension: movement in the first half of the day XX points up/down
# second dimension: movement in the second half of the day YY points up/down

rm(list=ls())
dax_2018 <- read.csv("dax_2018.csv")

# want to obtain time series for each date, thus pivot minutes to columns
dax_2018_piv <- dax_2018 %>% pivot_wider(names_from = time, values_from = close)

# look for missing values: only keep complete time series
miss_val <- 0
for(i in 1:length(dax_2018_piv$date)){miss_val[i] <- (sum(is.na(dax_2018_piv[i,])))}

dax_2018_piv_clean <- dax_2018_piv[miss_val==0,]

# sort colnames in ascending minutes
sort_cols <- sort(colnames(dax_2018_piv_clean)[2:841])
all_cols <- c("date", sort_cols)

dax_series <- dax_2018_piv_clean[,all_cols]

rm(list=ls()[-5]) # keep only dax_series
...
# scatter plot of am_movement vs. pm_movement
ggplot(dax_movement, mapping = aes(x=am_movement, y=pm_movement)) +
  geom_point()

dax_movement[,2:3]

# kmeans using Lloyd-algorithm (repeat short explaination)
km <- kmeans(dax_movement[,2:3], centers = 3, algorithm = "Lloyd", iter.max = 30, nstart = 20)
# no convergence with iter.max = 10 (default), set to 30

# check, if we get close to absolute minimum of total within-cluster sum of squares
i = 1
for (i in 1:20){
...
# can get stuck, but only once in 20 starts. so nstart = 20 is more than enough

# need to specify the optimal number of clusters:
...
# extract wss for 1-15 clusters and put LABELS to x- and y-axes
...
# plot ideal number of clusters: 3 (ellbow)
km <- kmeans(dax_movement[,2:3], centers = 3, algorithm = "Lloyd", iter.max = 30, nstart = 20)
...
# further analysis:
# without clustering we are left with simply dividing am movements in POS (up) and NEG (down)
# but means of pm_movement are close to zero, thus K.O. strategy cannot be applied
...
# can we make use of the clustering? 
# e.g. cluster 1: am movement negativ but pm movement randomly distributed

cluster1 <- dax_movement_cluster %>% filter(cluster == 1)
mean(cluster1$am_movement)
mean(cluster1$pm_movement)

# conclusion: clustering results are not usefull for applying K.O. strategy
...
Highlight what you think are the most interesting results. Also reflect on what you have learned during the project, or what types of problems you faced.
